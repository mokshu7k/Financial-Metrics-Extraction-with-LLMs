# src/data_preprocessing/chunking.py
import json
import re
...
class FinancialDocumentChunker:
    ...
    def save_chunks(self, chunks: Dict[str, List[DocumentChunk]], output_suffix: str = ""):
        ...
        chunks_dir = self.processed_data_dir / "chunks"
        ...
        if all_chunk_metadata:
            ...
            with open(chunks_dir / f"chunking_stats{output_suffix}.json", 'w') as f:
    ...
    def generate_chunking_statistics(self, metadata_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'token_statistics': {
                'mean': metadata_df['token_count'].mean(),
                'median': metadata_df['token_count'].median(),
                'min': metadata_df['token_count'].min(),
                'max': metadata_df['token_count'].max(),
                'std': metadata_df['token_count'].std()
            },
            'financial_data_chunks': metadata_df['contains_financial_data'].sum(),
        ...
        for chunk_type in [ChunkType.FIXED_SIZE, ChunkType.PARAGRAPH, ChunkType.SECTION]:
            ...
            if total_chunks > 0:
                ...
                comparison_results[chunk_type.value] = {
                    ...
                    'avg_tokens_per_chunk': metadata_df['token_count'].mean(),
                    'financial_data_coverage': metadata_df['contains_financial_data'].mean(),
                    'chunk_size_variance': metadata_df['token_count'].std()
                }
        
        # Save comparison results
        with open(self.processed_data_dir / "chunking_comparison.json", 'w') as f:
...
# Utility functions for chunk analysis and optimization
...
def analyze_chunk_quality(chunks_metadata_path: Path) -> Dict[str, Any]:
    ...
    analysis = {
        'size_distribution': {
            'under_100_tokens': (metadata_df['token_count'] < 100).sum(),
            '100_500_tokens': ((metadata_df['token_count'] >= 100) & (metadata_df['token_count'] < 500)).sum(),
            '500_1000_tokens': ((metadata_df['token_count'] >= 500) & (metadata_df['token_count'] < 1000)).sum(),
            'over_1000_tokens': (metadata_df['token_count'] >= 1000).sum()
        },
        'content_quality': {
            'chunks_with_financial_data': metadata_df['contains_financial_data'].sum(),
            'empty_or_short_chunks': (metadata_df['token_count'] < 50).sum(),
            'very_long_chunks': (metadata_df['token_count'] > 2000).sum()
        },
        'coverage': {
            'companies_covered': metadata_df['company'].nunique(),

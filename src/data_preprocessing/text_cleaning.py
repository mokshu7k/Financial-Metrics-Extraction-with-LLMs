...
class DocumentMetadata:
    ...
    quarter: str
    year: int
    document_type: str  # 'financial' or 'transcript'
    file_path: str
    text_length: int
...
class FinancialTextCleaner:
    """
    Cleans and preprocesses financial documents (filings and transcripts) 
    for the RAG research pipeline.
    """
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.data_dir = Path(self.config.get('paths.data_root', 'data'))
        self.raw_dir = Path(self.config.get('paths.raw_data', 'data/raw'))
        self.processed_dir = Path(self.config.get('paths.processed_data', 'data/processed'))
        
        # Ensure directories exist
        ensure_directory(self.processed_dir)
        
        # Setup logging
        log_file = self.config.get('paths.logs', 'results/logs') + '/pipeline.log'
        log_level = self.config.get('logging.level', 'INFO')
        setup_logging(level=log_level, log_file=log_file)
        self.logger = get_logger(__name__)
        
        # Get cleaning parameters from config
        self.cleaning_config = self.config.get('preprocessing.cleaning', {})
        
        # File parsing patterns
        self.file_pattern = re.compile(r'([A-Za-z\s]+)\s+(q[1-4])\s+(\d{4})\.pdf', re.IGNORECASE)
    ...
    @timer("PDF text extraction")
    def extract_pdf_text(self, file_path: Path) -> str:
        """Extract text from PDF using pdfplumber for better handling."""
        try:
            text = ""
    ...
    def parse_filename(self, filename: str) -> Optional[Tuple[str, str, int]]:
        ...
        if match:
            company = match.group(1).strip().upper().replace(" ", "_")
    ...
    def process_document(self, file_path: Path, doc_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metadata = DocumentMetadata(
                ...
                processing_timestamp=get_timestamp("%Y-%m-%d %H:%M:%S")
            )
    ...
    @timer("Processing all documents")
    ...
    def save_processed_documents(self, results: Dict[str, List[Dict[str, Any]]]):
        ...
        for doc_type, documents in results.items():
            ...
            # Create subdirectory
            output_dir = ensure_directory(self.processed_dir / doc_type)
            ...
            for doc in documents:
                ...
                # Save full document
                safe_json_save(doc, output_dir / filename)
    ...
    def generate_processing_report(self, results: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        # Save report
        safe_json_save(report, self.processed_dir / "processing_report.json")
        
        return report
    
    @timer("Full preprocessing pipeline")
...
if __name__ == "__main__":
    cleaner = FinancialTextCleaner(config_path="config.yaml")
